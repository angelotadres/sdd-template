...
- Skills live in `.claude/skills/`. The `bootstrap` skill drafts the constitution on a fresh repo; the `feature-spec` skill automates new-phase kickoff from `AGENTS.md`; the `changelog` skill maintains `CHANGELOG.md`.
...
- Do not write code for a phase before its spec trio exists under `specs/initiatives/<initiative-name>/YYYY-MM-DD-<phase-name>/`.
